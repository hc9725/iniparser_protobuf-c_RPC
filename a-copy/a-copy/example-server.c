...
static void load_database(const char *filename)
{
	...
	if (isspace(buf[0])) {
	    ...
	    char *start = buf + 1;
	    if (n_people == 0)
		die("error on %s, line %u: line began with a space, but no person's name preceded it", filename, line_no);
		...
		tmp.type =
		    start[0] ==
		    'm' ? FOO__PERSON__PHONE_TYPE__MOBILE : start[0] ==
		    'h' ? FOO__PERSON__PHONE_TYPE__HOME :
		    FOO__PERSON__PHONE_TYPE__WORK;
		tmp.number = xstrdup(peek_next_token(start));
		person->phone =
		    xrealloc(person->phone,
		...
		*pn = tmp;
		...
		people_alloced *= 2;
		people =
		    xrealloc(people, people_alloced * sizeof(Foo__Person));
	    }
...
static void load_database2(const char *filename)
{
	...
	else 
	{
		...
		array_alloced *= 2;
		Array =
		    xrealloc(array, array_alloced * sizeof(Foo__Array));
	    }
	    n_array++;
	    array = Array + n_array - 1;
